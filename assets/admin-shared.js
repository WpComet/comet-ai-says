            ...
            if (this.results.errors === 0) {
                resultsHtml = `
                    <div class="notice notice-success">
                        <p><strong>${wpcmt_aisays.i18n.completed}</strong></p>
                        <p>${wpcmt_aisays.i18n[actionText + "_count"].replace("%d", this.results.success)}</p>
                    </div>
                `;
            } else {
                resultsHtml = `
                    <div class="notice notice-warning">
                        <p><strong>${wpcmt_aisays.i18n.completed}</strong></p>
                        <p>${wpcmt_aisays.i18n[actionText + "_count"].replace("%d", this.results.success)}</p>
                        <p>${wpcmt_aisays.i18n[actionType + "_error_specific"].replace("%s", this.results.errors)}</p>
                `;
    ...
    $(document).on("click", "#doaction, #doaction2", function (e) {
        ...
        if (action === "bulk_generate" || action === "bulk_delete") {
            ...
            const confirmMessage =
                actionType === "delete"
                    ? wpcmt_aisays.i18n.bulk_delete_confirm.replace("%d", productIds.length)
                    : wpcmt_aisays.i18n.bulk_confirm.replace("%d", productIds.length);
